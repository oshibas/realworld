class UsersController < ApplicationController
  # UsersControllerを作成し、ApplicationControllerを継承する
  skip_before_action :authorize_request, only: [:create, :show]
  # ユーザーの作成の場合はリクエストの認可をスキップする
  ...
  def create # ユーザーの作成
    ...
    if @user.save # ユーザーの保存に成功した場合
      ...
      # 保存に成功した場合はユーザーをJSON形式でレスポンスとして返す
    else # ユーザーの保存に失敗した場合
      render json: { errors: @user.errors }, status: :unprocessable_entity
      # 保存に失敗した場合はエラーメッセージをJSON形式でレスポンスとして返す
    end
  end
  ...
  def show # ユーザーの詳細
    ...
    begin # JWTのデコード
      ...
      # トークンのデコードに失敗した場合はエラーメッセージをJSON形式でレスポンスとして返す
    end

    user_id = decoded_token.first["sub"]
    # デコードしたトークンからユーザーIDを取得する
    user = User.find(user_id)
    # ユーザーIDからユーザーを取得する

    if user.nil? # ユーザーが見つからない場合
      render json: { message: 'Unauthorized' }, status: :unauthorized
      # エラーメッセージをJSON形式でレスポンスとして返す
    else # ユーザーが見つかった場合
